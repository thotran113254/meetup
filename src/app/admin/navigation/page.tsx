  ...
  return (
    <div className="w-full space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold">Điều hướng</h1>
          <p className="text-sm text-[var(--color-muted-foreground)] mt-1">{items.length} mục</p>
        </div>
      ...
      <div className="rounded-xl border border-[var(--color-border)] bg-[var(--color-card)] overflow-hidden">
        {loading ? (
          ...
          <div className="py-12 text-center text-sm text-[var(--color-muted-foreground)]">Chưa có mục điều hướng nào</div>
        ) : (
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[var(--color-border)] bg-[var(--color-muted)]">
                <th className="px-4 py-3 text-left font-medium text-[var(--color-muted-foreground)]">Nhãn</th>
                <th className="px-4 py-3 text-left font-medium text-[var(--color-muted-foreground)] hidden sm:table-cell">Đường dẫn</th>
                <th className="px-4 py-3 text-left font-medium text-[var(--color-muted-foreground)] hidden md:table-cell">Thứ tự</th>
            ...
            <tbody>
              {topLevel.map((item) => (
                <React.Fragment key={item.id}>
                  <NavRow item={item} onEdit={openEdit} onDelete={setDeleteTarget} indent={false} />
      ...
      <AdminConfirmDialog
        ...
        title="Xóa mục điều hướng"
        description={`Xóa mục "${deleteTarget?.label}"? Các mục con cũng sẽ mất liên kết cha.`}
