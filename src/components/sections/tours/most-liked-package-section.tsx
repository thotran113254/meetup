...
function FeaturedCard({ tour, showOverlay }: { tour: TourPackage; showOverlay?: boolean }) {
          ...
          <div className="bg-white rounded-xl p-4 max-h-[344px] overflow-y-auto flex flex-col gap-3">
            <div className="flex items-center justify-between">
              <p className="text-[14px] font-bold text-[#1D1D1D] tracking-[0.14px]">
                ...
                <span className="text-[#3BBCB7]">{String(tour.flights).padStart(2, "0")}</span>
              </p>
...
export function MostLikedPackageSection({ sectionTitle, tours }: Props) {
  const displayTitle = sectionTitle || "Most Liked Package";
  ...
  return (
    <section className="py-8 md:py-12 bg-white relative overflow-x-hidden">
      <div className="max-w-[1400px] mx-auto px-4 lg:px-[100px] flex flex-col gap-4 md:gap-5">
        <ScrollReveal>
          <h2 className="text-[20px] md:text-[32px] font-bold leading-[1.2] text-[#1D1D1D]">{displayTitle}</h2>
        </ScrollReveal>
