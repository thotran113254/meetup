  ...
  return (
    <div>
      ...
      <nav className="mt-3 px-4 md:px-0" aria-label="Breadcrumb">
        <ol className="flex items-center gap-1 md:gap-1.5 text-[10px] md:text-[12px] font-medium">
          {BREADCRUMB_ITEMS.map((item) => (
            <li key={item.label} className="flex items-center gap-1 md:gap-1.5 shrink-0">
              <Link
                href={item.href}
              ...
              <ChevronRight className="w-2.5 h-2.5 md:w-3 md:h-3 text-[#BDBDBD] shrink-0" />
            </li>
          ))}
          <li className="text-[#1D1D1D] truncate min-w-0">Name of packages</li>
        </ol>
      </nav>
    </div>
  );
